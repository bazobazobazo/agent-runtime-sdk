...
The manual workflow is tag-bound, uses the `next` prerelease channel, pins the
trusted-publishing Node/npm toolchain, requires explicit publication confirmation
and the protected `npm-release` environment, and grants OIDC only to its
publication job. The first creation of the six npm package records requires the
documented short-lived-token bootstrap and immediate revocation; subsequent
publication must use OIDC, with no long-lived npm token. External
