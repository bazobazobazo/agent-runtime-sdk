# Changelog

## 0.1.0-alpha.7

- Synchronized fixed-version release with coverage for progress, recovery, and schedule delivery behavior.

## 0.1.0-alpha.6

- Synchronized fixed-version release for opt-in OpenClaw history compatibility.

## 0.1.0-alpha.5

- Synchronized fixed-version release for OpenClaw post-wait history recovery.

## 0.1.0-alpha.4

- Synchronized fixed-version release for OpenClaw legacy history recovery.

## 0.1.0-alpha.3

- Model OpenClaw terminal wait-cache expiry and exact run-correlated persistent
  history for deterministic v3/v4 recovery testing.

## 0.1.0-alpha.1 — release candidate

- Initial synchronized pre-alpha release candidate.
- See the repository changelog and release notes for features, security posture, migration guidance, and known limitations.
